...
class DoctorBloc extends Bloc<DoctorEvent, DoctorState> {
  ...
  //   emit(DoctorsLoading());
  //   try {
  //     final List<Doctor> allDoctors = (state is DoctorsLoaded)
  //         ? (state as DoctorsLoaded).doctors
  //         : []; // Get the current list of doctors from the state

  //     final List<Doctor> searchResults = allDoctors.where((doctor) {
  //       final String doctorusername = doctor.username.toLowerCase();
  //       final String query = event.query.toLowerCase();
  //       return doctorusername.contains(query);
  //     }).toList();

  //     emit(SearchResultsLoaded(searchResults));
  //   } catch (e) {
  //     emit(SearchFailure(GenericFailure("Failed to search doctors")));
  //   }
  // }
